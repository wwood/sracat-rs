...
# randomly sample 10,000 spots (reproducible; reads only those rows)
sracat-rs --accept-singles --sample 10000 run.sra >subsample.fasta

# draw a different sample with a different seed
sracat-rs --accept-singles --sample 10000 --seed 7 run.sra >subsample2.fasta
```

### Random sampling (`--sample`)

`--sample N` draws `N` spots at random rather than reading the whole run. The
ncbi-vdb cursor is random-access, so `sracat-rs` seeks straight to the chosen
